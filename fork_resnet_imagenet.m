function [net, derOutputs] = fork_resnet_imagenet(net, varargin)
% Based on Zhizhong Li's code
% Modified by F. Castro for End-to-End Incremental Learning. ECCV2018

opts.newtaskdim = 100; % # output of last layer
...
index = strfind({net.layers.name}, 'error5');
index = find(not(cellfun('isempty', index)));
net.removeLayer(net.layers(index(1)).name);
...
net.addLayer('error5', dagnn.Loss('loss', 'topkerror', 'opts', {'topK', 5}), ...
  {'softmax_global','global_label'}, 'error5') ;
